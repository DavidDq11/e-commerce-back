...
export const findUserByEmail = async (email) => {
  const query = 'SELECT id, first_name, last_name, email, password, phone, city, state, address, admin FROM users WHERE email = $1';
...
export const findUserById = async (id) => {
  const query = `
    SELECT id, first_name, last_name, email, phone, city, state, address, admin
    FROM users
    WHERE id = $1
  `;
